...
export default async function handler(req, res){
  ...
  try{
    if (req.method === 'GET'){
      try {
        ...
        if (rows && rows.length > 0) {
          const q2 = await pool.query('SELECT id, name, code FROM analytics')
          ...
          try {
            const q3 = await pool.query('SELECT id, ebrigade_activity_type FROM activities')
      ...
      try {
        ...
        try {
          const insertQ = await pool.query(
            ...
            [analytic_id, analytic_name||null, analytic_code||null, typeToSave||null, typeToSave||null, date||null, remuneration_infi||null, remuneration_med||null]
          )
          const insertId = (insertQ && insertQ.rows && insertQ.rows[0]) ? insertQ.rows[0].id : null
          ...
          const row = (selQ && selQ.rows && selQ.rows[0]) ? selQ.rows[0] : null
          ...
          if (err && (err.code === '42703' || (err.message && err.message.includes('ebrigade_activity_type')))) {
            const insertQ2 = await pool.query(
              ...
              [analytic_id, analytic_name||null, analytic_code||null, typeToSave||null, date||null, remuneration_infi||null, remuneration_med||null]
            )
            const insertId2 = (insertQ2 && insertQ2.rows && insertQ2.rows[0]) ? insertQ2.rows[0].id : null
            ...
            const row2 = (selQ2 && selQ2.rows && selQ2.rows[0]) ? selQ2.rows[0] : null
    ...
    console.error('[api/admin/activities] Error:', err.message)
