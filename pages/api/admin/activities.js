...
export default async function handler(req, res){
  ...
  try{
    if (req.method === 'GET'){
      const [rows] = await pool.query('SELECT a.id, a.analytic_id, an.name as analytic_name, an.code as analytic_code, a.pay_type, a.ebrigade_activity_type, a.date, a.remuneration_infi, a.remuneration_med, a.created_at FROM activities a LEFT JOIN analytics an ON a.analytic_id = an.id ORDER BY a.id DESC')
