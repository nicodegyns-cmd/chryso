...
export default async function handler(req, res){
  ...
  try{
    ...
    if (req.method === 'POST'){
      try {
        const { analytic_id, analytic_name, analytic_code, pay_type, ebrigade_activity_type, date, remuneration_infi, remuneration_med, ebrigade_analytics } = req.body || {}
        ...
        try {
          ...
          const insertId = (insertQ && insertQ.rows && insertQ.rows[0]) ? insertQ.rows[0].id : (Array.isArray(insertQ) && insertQ[0] && insertQ[0].insertId) ? insertQ[0].insertId : null
