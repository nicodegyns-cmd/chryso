...
export default async function handler(req, res){
  ...
  try{
    ...
    if (activity_id) {
      try{
        const result_act_outer = await pool.query('SELECT analytic_id FROM activities WHERE id = $1 LIMIT 1', [activity_id])
    ...
    const q_rows = await pool.query('SELECT * FROM prestations WHERE id = $1 LIMIT 1', [id])
