...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      const [[row]] = await pool.query('SELECT id, name, analytic_type, code, entite, distribution, description, is_active, created_by, created_at FROM analytics WHERE id = ?', [id])
    ...
    if (req.method === 'PUT' || req.method === 'PATCH') {
      ...
      params.push(id)
      const sql = `UPDATE analytics SET ${updates.join(', ')} WHERE id = ?`
    ...
    if (req.method === 'DELETE') {
      await pool.execute('DELETE FROM analytics WHERE id = ?', [id])
