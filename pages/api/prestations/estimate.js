...
export default async function handler(req, res){
  ...
  try{
    ...
    if (analytic_id) {
      try{
        const q_allActs = await pool.query('SELECT pay_type, remuneration_infi, remuneration_med, date FROM activities WHERE analytic_id = ? ORDER BY date DESC', [analytic_id])
    ...
    if ((!resolvedRole || String(resolvedRole).trim() === '') && user_email){
      try{
        const q_urows = await pool.query('SELECT role FROM users WHERE LOWER(email) = ? LIMIT 1', [(user_email||'').toLowerCase()])
