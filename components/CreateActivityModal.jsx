...
export default function CreateActivityModal({ open, onClose, onCreate, initial, onUpdate }){
  ...
  const [hourEntryType, setHourEntryType] = useState('auto') // 'auto', 'garde', 'simple'
  ...
  useEffect(() => {
    if (initial) {
      ...
      setHourEntryType(initial.hour_entry_type || 'auto')
  ...
  async function submit(e){
    ...
    try {
      ...
      const payload = {
        ...
        hour_entry_type: hourEntryType === 'auto' ? null : hourEntryType,
        date: date || null,
      ...
      <div className="modal create-user-modal" style={{maxWidth:'600px',maxHeight:'90vh',overflow:'auto'}}>
        ...
        <div className="modal-body" style={{padding:'24px'}}>
          <form className="create-user-form" onSubmit={submit} style={{display:'flex',flexDirection:'column',gap:20}}>
            ...
            {/* Section: Type de saisie des heures */}
            <div style={{borderLeft:'3px solid #6366f1',paddingLeft:16}}>
              <label style={{display:'block',marginBottom:4}}>
                ...
                <select
                  ...
                  style={{width:'100%',padding:'10px 12px',border:'1px solid #d1d5db',borderRadius:6,fontSize:14}}
                >
                  <option value="auto">Auto (détection par pay_type)</option>
                ...
                <small style={{display:'block',marginTop:6,color:'#6b7280'}}>Détermine le layout du modal de saisie pour les utilisateurs</small>
              </label>
            </div>
