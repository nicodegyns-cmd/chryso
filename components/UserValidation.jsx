...
export default function UserValidation() {
  ...
  async function handleValidate(userId) {
    if (!form.role) { alert('Veuillez sÃƒÂ©lectionner un rÃƒÂ´le'); return }
    ...
    try {
      ...
      setViewing(null)
      alert('Ã¢Å“â€œ Utilisateur validÃƒÂ© et activÃƒÂ© avec succÃƒÂ¨s!')
    } catch (e) {
  ...
  async function handleReject(userId) {
    if (!confirm('Rejeter cet utilisateur? Son compte sera supprimÃƒÂ©.')) return
    try {
  ...
  function getMissingFields(user) {
    const missing = []
    if (!user.first_name || !user.last_name) missing.push('PrÃƒÂ©nom/Nom')
    if (!user.telephone) missing.push('TÃƒÂ©lÃƒÂ©phone')
  ...
  const catStyle = {
    ready:      { border: '#10b981', bg: '#f0fdf4', badge: '#d1fae5', badgeText: '#065f46', label: 'Ã¢Å“â€œ PrÃƒÂªt ÃƒÂ  valider' },
    incomplete: { border: '#f59e0b', bg: '#fffbeb', badge: '#fef3c7', badgeText: '#b45309', label: 'Ã¢Å¡Â Ã¯Â¸Â Infos manquantes' },
    waiting:    { border: '#6b7280', bg: '#f9fafb', badge: '#f3f4f6', badgeText: '#374151', label: 'Ã°Å¸â€œÂ§ Pas encore inscrit' },
  }

  const tabs = [
    { key: 'all',        label: 'Tous',               count: counts.all },
    { key: 'ready',      label: 'Ã¢Å“â€œ PrÃƒÂªts ÃƒÂ  valider',  count: counts.ready },
    { key: 'incomplete', label: 'Ã¢Å¡Â Ã¯Â¸Â Infos manquantes', count: counts.incomplete },
    { key: 'waiting',    label: 'Ã°Å¸â€œÂ§ Invitation seule', count: counts.waiting },
  ]

  if (loading) return <div style={{ padding: 24, color: '#6b7280' }}>ChargementÃ¢â‚¬Â¦</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        <div style={{ padding: '14px 16px', background: '#f0fdf4', borderRadius: 8, border: '1px solid #bbf7d0', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: '#16a34a', marginTop: 2 }}>PrÃƒÂªts ÃƒÂ  valider</div>
        </div>
        ...
        <div style={{ padding: '14px 16px', background: '#f9fafb', borderRadius: 8, border: '1px solid #e5e7eb', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 12, color: '#6b7280', marginTop: 2 }}>Invitation envoyÃƒÂ©e</div>
        </div>
      </div>
      ...
      {/* List */}
      {displayed.length === 0 ? (
        <div style={{ padding: 32, textAlign: 'center', color: '#9ca3af' }}>Aucun utilisateur dans cette catÃƒÂ©gorie</div>
      ) : (
              ...
              >
                <div style={{ flex: 1, padding: '14px 16px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                    <span style={{ fontSize: 12, color: '#6b7280' }}>Ã°Å¸â€œÂ§ {user.email}</span>
                    {user.telephone && <span style={{ fontSize: 12, color: '#6b7280' }}>Ã°Å¸â€œÅ¾ {user.telephone}</span>}
                    {user.niss && <span style={{ fontSize: 12, color: '#6b7280' }}>Ã°Å¸ÂªÂª NISS Ã¢Å“â€œ</span>}
                    {user.bce && <span style={{ fontSize: 12, color: '#6b7280' }}>Ã°Å¸ÂÂ¢ BCE Ã¢Å“â€œ</span>}
                    {user.account && <span style={{ fontSize: 12, color: '#6b7280' }}>Ã°Å¸â€™Â³ Compte Ã¢Å“â€œ</span>}
                  </div>
                  {missing.length > 0 && (
                    <div style={{ marginTop: 6, fontSize: 11, color: '#b45309' }}>
                      Manque : {missing.join(' Ã‚Â· ')}
                    </div>
                  )}
                </div>
                <div style={{ padding: '14px 16px', fontSize: 18, color: cat === 'ready' ? '#10b981' : '#d1d5db' }}>Ã¢â‚¬Âº</div>
              </div>
            )
          })}
        </div>
        ...
        <div style={{ position: 'fixed', left: 0, top: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 560, maxHeight: '92vh', overflow: 'auto', boxShadow: '0 20px 40px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button onClick={() => setViewing(null)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#9ca3af', lineHeight: 1 }}>Ã¢Å“â€¢</button>
            </div>
            ...
            <div style={{ padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {viewing.onboarding_status === 'pending_signup' && (
                <div style={{ background: '#fef3c7', border: '1px solid #fcd34d', borderRadius: 8, padding: 12, fontSize: 12, color: '#92400e' }}>
                  Ã¢ÂÂ³ Cet utilisateur n'a pas encore complÃƒÂ©tÃƒÂ© son profil. Son invitation est en attente.
                </div>
              )}
              {viewing.onboarding_status === 'pending_validation' && getMissingFields(viewing).length > 0 && (
                <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: 8, padding: 12, fontSize: 12, color: '#c2410c' }}>
                  Ã¢Å¡Â Ã¯Â¸Â <strong>Ãƒâ‚¬ complÃƒÂ©ter avant validation :</strong> {getMissingFields(viewing).join(', ')}
                </div>
              )}
              ...
              {(viewing.telephone || viewing.fonction || viewing.company || viewing.address) && (
                <div style={{ background: '#f9fafb', borderRadius: 8, padding: 14, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  {viewing.telephone && <div><div style={{ fontSize: 11, fontWeight: 600, color: '#9ca3af' }}>TÃƒÂ©lÃƒÂ©phone</div><div style={{ fontSize: 13, color: '#1f2937' }}>{viewing.telephone}</div></div>}
              ...
              <div style={{ display: 'grid', gap: 12 }}>
                <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>RÃƒÂ´le *</span>
                  <select value={form.role} onChange={e => setForm({ ...form, role: e.target.value })} style={{ padding: '9px 12px', border: '1px solid #d1d5db', borderRadius: 6, fontSize: 14 }}>
                    <option value="">-- SÃƒÂ©lectionner un rÃƒÂ´le --</option>
                    <option value="INFI">Infirmier</option>
                    <option value="MED">MÃƒÂ©decin</option>
                    <option value="moderator">ModÃƒÂ©rateur</option>
                    <option value="comptabilite">ComptabilitÃƒÂ©</option>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                  <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    ...
                    <input type="text" value={form.niss} onChange={e => setForm({ ...form, niss: e.target.value })} style={{ padding: '9px 12px', border: `1px solid ${form.niss ? '#10b981' : '#d1d5db'}`, borderRadius: 6, fontSize: 14 }} placeholder="NumÃƒÂ©ro NISS" />
                  </label>
                  <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: '#374151' }}>BCE</span>
                    <input type="text" value={form.bce} onChange={e => setForm({ ...form, bce: e.target.value })} style={{ padding: '9px 12px', border: `1px solid ${form.bce ? '#10b981' : '#d1d5db'}`, borderRadius: 6, fontSize: 14 }} placeholder="NumÃƒÂ©ro BCE" />
                  </label>
                </div>
                <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <div style={{ display: 'flex', gap: 10, paddingTop: 8, borderTop: '1px solid #e5e7eb' }}>
                <button onClick={() => handleValidate(viewing.id)} disabled={validating[viewing.id]} style={{
                  ...
                  {validating[viewing.id] ? 'Ã¢ÂÂ³ ValidationÃ¢â‚¬Â¦' : 'Ã¢Å“â€œ Valider le compte'}
                </button>
